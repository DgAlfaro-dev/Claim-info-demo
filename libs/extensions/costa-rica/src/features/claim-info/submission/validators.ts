...
export const validateCostaRicanRuc: PreSubmitValidator = async (
  ...
  console.log('[validateCostaRicanRuc] RUC recibido:', ruc);
  console.log('[validateCostaRicanRuc] Datos completos del Owner:', context.formData?.claimInfo?.policy?.Owner);

  if (ruc) {
    if (!isValidCostaRicanRuc(ruc)) {
      errors.ruc = 'El RUC de Costa Rica debe tener el formato correcto (9 o 10 dígitos)';
      console.log('[validateCostaRicanRuc] RUC inválido:', ruc);
    }
  } else {
